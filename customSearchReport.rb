...
class CustomSearchReport < Qt::Dialog
def initialize
super
# Providing Title for Report window
setWindowTitle("Customers Custom Report")
puts "busy with customers custom report"
...
def genReport(sDate,lDate)
puts "Busy with generating Custom Report"
puts sDate
puts lDate
@qry = @base.getCustomReport(sDate,lDate)
puts @qry.size
if(@qry.size!=0)
@tab.setRowCount(@qry.size)
@row = 0
